...
@cuda.jit('void(uint16[:,:,:], float32, float32)')
def collision_kernel_start(ftemp, tau_inv, v):
...
# cuda function to calculate f after collision step
@cuda.jit('void(uint16[:,:,:], uint16[:,:,:], float32, float32)')
def collision_kernel_dia(ftemp, dcur, tau_inv, v):
...
def collision_kernel(ftemp, tau_inv):
...
@cuda.jit('void(uint16[:,:,:], uint16[:,:,:], uint16[:,:,:], uint16[:,:,:])')
...
# thread function to handle streaming step
@cuda.jit('void(uint16[:,:,:], uint16[:,:,:], uint16[:,:,:], uint16[:,:,:])')
def propagate_kernel_new(dtemp, dcur, dabove, dbelow):
...
def propagate_start(ftable, ftable_empty, blockspergrid, threadsperblock, gpu_id, end
    ...
    collision_kernel_start[blockspergrid, threadsperblock, stream1](dtemp, tau_inv, out_u_lb)

    dnext = cuda.to_device(ftable[2, :500], stream=stream2)

    stream1.synchronize()
    dtemp.copy_to_host(ftable_empty[0, :500], stream=stream3)
    ...
    for i in range(1, end):
        ...
        if i > i4:
            stream2.synchronize()
            propagate_kernel_new[blockspergrid, threadsperblock, stream1](dtemp, dcur, dabove, dbelow)
            collision_kernel[blockspergrid, threadsperblock, stream1](dtemp, tau_inv)

        elif i1 < i < i2:
            stream2.synchronize()
            propagate_kernel[blockspergrid, threadsperblock, stream1](dtemp, dcur, dabove, dbelow)
            collision_kernel_dia[blockspergrid, threadsperblock, stream1](dtemp, dcur, tau_inv, dia_u_lb)
        ...
        stream1.synchronize()
        dtemp.copy_to_host(ftable_empty[i, :dtemp.shape[0]], stream=stream3)
...
# thread function to handle streaming step
def propagate(ftable, ftable_empty, height, blockspergrid, threadsperblock, gpu_id, start, end, tau_inv):
    ...
    for i in range(start, end):
        ...
        # if i == height - 1:
        #     stream2.synchronize()
        #     propagate_kernel[blockspergrid, threadsperblock, stream1](dtemp, dcur, dcur, dbelow)
        #
        # else:
        stream2.synchronize()
        propagate_kernel_new[blockspergrid, threadsperblock, stream1](dtemp, dcur, dabove, dbelow)
...
def conti(duration, starttime, lu, dia_u_lb_max, out_u_lb, tau_inv, ts, ftable
          , interval, frequency, dia_delay, out_folder):

    ftable_empty = np.zeros((ftable.shape[0], ftable.shape[1], ftable.shape[2], 19), np.uint16)
    ...
    dia_delay -= ts
    # for gpu_id in range(1, device_num):
    #     start[gpu_id] = (gpu_id - 1) * (ftable.shape[0] - 1300) // (device_num - 1) + 1300
    #     end[gpu_id] = gpu_id * (ftable.shape[0] - 1300) // (device_num - 1) + 1300
    #
    # end[0] = 1300
    ...
    while t < endtime:
        ...
        with cf.ThreadPoolExecutor(max_workers=device_num) as executor:
            ...
            for i in range(1, device_num):
                executor.submit(propagate, ftable, ftable_empty, ftable.shape[0], blockspergrid,
                                threadsperblock, i, start[i], end[i], tau_inv)

            executor.submit(propagate_start, ftable, ftable_empty, blockspergrid,
                            threadsperblock, 0, end[0], dia_u_lb, out_u_lb, tau_inv)
